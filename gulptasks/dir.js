...
module.exports = function(config) {
  var html = [
    ...
    tmpHtml = '',
    length = '/src/views/'.length,
    collector = listdir('./src/views'),
  ...
  function listdir(dir) {
    ...
    files.forEach(function(file) {
      ...
      var url = absolutePath.substring(absolutePath.indexOf('./src/views/') + length + 1);

      if (stats.isDirectory() && (stats.isDirectory() !== '.' || stats.isDirectory() !== '..')) {
        collector['child'].push(listdir(absolutePath));
      } else {
        collector['child'].push({'name': path.basename(absolutePath), 'type': 'file', 'url': url});
      }
    });
  ...
  var out = fs.createWriteStream('./src/views/TmTIndex.html', {encoding: "utf8"});
